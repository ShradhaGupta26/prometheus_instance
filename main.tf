...
resource "aws_iam_instance_profile" "prometheus_profile" {
  count = var.iam_instance_profile == "" ? 1 : 0
  name  = "${var.project_name_prefix}-prometheus-profile-tanya"
  role = aws_iam_role.prometheus_role[0].name
...
vars = {
 ...
 mongo_primary_private_ip= var.mongo_primary_private_ip
 mongo_secondary_private_ip_1= local.mongo_secondary_private_ip_1
 mongo_secondary_private_ip_2= local.mongo_secondary_private_ip_2
}
}
